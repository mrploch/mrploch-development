...
targets:
  ...
  copilot:
    ...
    output:
      ...
      # Advisory only — Coding Agent MCP is configured via the GitHub Settings UI,
      # never read from a committed file. The generator emits a paste-ready advisory.
      recommended_mcp_file: .github/copilot-mcp.recommended.md
    options:
...
- **Exception — snapshot-export artifacts.** The opt-in `.ai-context/*` snapshot files (see [`cloud-agents.md`](cloud-agents.md)) are exempt from the no-timestamp rule: they carry an explicit `snapshotted on YYYY-MM-DD` header because their content is inherently non-deterministic (ContextStream evolves between emits). These files **must** be excluded from `check` mode's drift detection. No other emitted file may contain a timestamp.
