...
description: Resolve every PR comment AND every SonarCloud platform issue on the current branch end-to-end. Triage each thread or issue by the seven-category model from `pr-checks-completion-gate.md`, consult Codex and Gemini in parallel for non-trivial cases, apply fixes, post evidence-backed replies, resolve threads, and re-poll until nothing unaddressed remains.
allowed-tools: Bash(git:*), Bash(gh:*), Read, Edit, Write, Glob, Grep, WebSearch, WebFetch, mcp__codex-cli__codex, mcp__codex-cli__review, mcp__gemini__gemini-analyze-code, mcp__gemini__gemini-brainstorm, mcp__gemini__gemini-query, mcp__github__pull_request_read, mcp__sonarqube-cloud__search_my_sonarqube_projects, mcp__sonarqube-cloud__list_pull_requests, mcp__sonarqube-cloud__search_sonar_issues_in_projects, mcp__sonarqube-cloud__search_security_hotspots, mcp__sonarqube-cloud__get_project_quality_gate_status, mcp__sonarqube-cloud__show_rule, mcp__sonarqube-cloud__change_sonar_issue_status, mcp__sonarqube-cloud__change_security_hotspot_status
---
...
1b. SonarCloud sweep    ─► fetch platform issues + hotspots via the sonarqube-cloud MCP server
2. Triage               ─► assign each thread AND each SonarCloud issue one of 7 categories
...
**In scope for triage:** every reviewThread where `isResolved=false AND isOutdated=false`, plus every issue-style comment whose latest activity is not by us, plus every review with `state IN (CHANGES_REQUESTED, COMMENTED)` whose body raises an open concern. Plus **every SonarCloud platform issue and security hotspot fetched in Phase 1b** — each one is triaged exactly like a reviewer thread.
...
## Phase 1b: SonarCloud Platform Issues (via MCP)

SonarCloud / SonarQube Cloud does **not** post one PR thread per finding. It posts a single summary comment ("Quality Gate passed/failed — N new issues") plus a `SonarQube Cloud` status check. The actual bugs, code smells, vulnerabilities, and security hotspots live in the SonarCloud platform — **replying to the summary comment does not address them.** Fetch them directly via the `sonarqube-cloud` MCP server (configured at workspace scope — see `mrploch/CLAUDE.md` § "SonarQube MCP Servers").

> A passing quality gate is **not** "zero issues" — a gate can pass with new issues that sit below its threshold. Always enumerate the issues; never trust the gate status alone.

### Resolve the project key and PR identifier

1. **Project key** — look in this order:
   - `.sonarlint/connectedMode.json` → `projectKey`.
   - `sonar.projectKey` in `sonar-project.properties`, or in `.github/workflows/*.yml` (the mrploch repos use the shared `build-test-sonar` action, which sets it).
   - Fallback: `mcp__sonarqube-cloud__search_my_sonarqube_projects(q="<repo-name>")`.
2. **PR identifier** — SonarCloud keys a PR by its GitHub PR number. Confirm with `mcp__sonarqube-cloud__list_pull_requests(projectKey="<key>")`, matching the source branch to `$BRANCH`.

### Fetch every open issue and hotspot for the PR

```
mcp__sonarqube-cloud__search_sonar_issues_in_projects(
    projects=["<project-key>"], pullRequestId="<PR_NUMBER>",
    issueStatuses=["OPEN", "CONFIRMED"])

mcp__sonarqube-cloud__search_security_hotspots(
    projectKey="<project-key>", pullRequest="<PR_NUMBER>",
    status=["TO_REVIEW"])

mcp__sonarqube-cloud__get_project_quality_gate_status(
    projectKey="<project-key>", pullRequest="<PR_NUMBER>")
```

Page through results (`p` / `ps`) until exhausted. For any rule you don't recognise, call `mcp__sonarqube-cloud__show_rule(key="<rule-key>")` to see what it flags and why.

**Every issue and hotspot returned is an in-scope item for Phase 2 triage** — treat each one exactly like a reviewer thread. Non-trivial ones go through the Phase 3 Codex + Gemini consultation like any other thread (Phase 3 already anticipates "a SonarCloud quality-gate complaint about a block of code"). Their resolution mechanics differ from GitHub threads — see Phase 7 § "SonarCloud platform issues — resolution".
...
### SonarCloud platform issues — resolution

SonarCloud platform issues (fetched in Phase 1b) have no GitHub thread to reply to. The seven categories still apply; the mechanics differ:

| Category | SonarCloud resolution |
|---|---|
| `VALID_ISSUE` / `SUGGESTION_ACCEPTED` | Fix in code (Phase 5) → push → the next SonarCloud scan auto-marks the issue `FIXED`. Verify after CI in Phase 8. |
...
| `SUGGESTION_DECLINED` / won't-fix | `mcp__sonarqube-cloud__change_sonar_issue_status(key="<issue-key>", status=["accept"])` — **pause and confirm with the user first.** |
| `ALREADY_FIXED` | Re-scan; if still listed, push a retrigger and re-check in Phase 8. |
...
| Security hotspot — reviewed safe | `mcp__sonarqube-cloud__change_security_hotspot_status(hotspotKey="<key>", status=["REVIEWED"], resolution=["SAFE"], comment="<evidence>")` — **pause and confirm with the user first.** |

When SonarCloud also posted a summary PR comment, reply on that comment with a roll-up so the platform work is visible on GitHub: *"Addressed N SonarCloud issues — M fixed in `<sha>`, K marked false-positive/accepted (rationale below)."* Then resolve that thread per the normal flow above.
...
1. Re-run the Phase 1 fetch **and the Phase 1b SonarCloud sweep**.
2. Check for **new** threads, **new** issue-style comments, **new** SonarCloud platform issues/hotspots, and **new** check runs since the last poll.
...
2. Every static-analysis bot has rendered a `success` verdict (stale ≠ pass). For SonarCloud / SonarQube Cloud, **additionally re-query the platform via the `sonarqube-cloud` MCP server** and confirm zero `OPEN`/`CONFIRMED` issues and zero `TO_REVIEW` hotspots for the PR — the green GitHub check alone is not proof.
...
- ❌ "Done — SonarCloud quality gate passed" (the gate status is not the issue list — query the platform via MCP)
...
| 3 | SonarCloud (platform issue `<key>`) | `<path>:<line>` | VALID_ISSUE | Fixed in `<sha>` — auto-marked `FIXED` on rescan |
| 4 | SonarCloud (hotspot `<key>`) | `<path>:<line>` | FALSE_POSITIVE | Reviewed `SAFE` with evidence (user-confirmed) |
| 5 | @<author> | conversation | OUT_OF_SCOPE | Tracked in #<follow-up-issue> |
...
- SonarCloud platform: <n> issues + <n> hotspots resolved; zero `OPEN`/`CONFIRMED` issues and zero `TO_REVIEW` hotspots remain
...
- Adding a new exclusion to `.codacy.yml`, `sonar-project.properties`, `.editorconfig` analyser severity, or `GlobalSuppressions.cs` — or marking a SonarCloud issue `falsepositive`/`accept`, or a security hotspot `SAFE`/`ACKNOWLEDGED`, via the `sonarqube-cloud` MCP server. State which findings would be silenced and why that is the right call.
...
- ❌ Replying to SonarCloud's summary PR comment without fetching and resolving the individual platform issues behind it.
- ❌ Trusting the SonarCloud quality-gate `pass` status as proof of zero issues — always enumerate via the `sonarqube-cloud` MCP server.
...
- `mcp__sonarqube-cloud__*` — fetch and resolve SonarCloud platform issues, hotspots, and quality-gate status for the PR: `search_my_sonarqube_projects`, `list_pull_requests`, `search_sonar_issues_in_projects`, `search_security_hotspots`, `get_project_quality_gate_status`, `show_rule`, `change_sonar_issue_status`, `change_security_hotspot_status`.
